...
st.set_page_config(page_title="Smart Panchayat - Sarpanch Dashboard", page_icon="🏛️", layout="wide")

# Sidebar
st.sidebar.title("🏛️ Gram Panchayat Portal")
st.sidebar.markdown("**Alandi Gram Panchayat**, Pune")

lang = st.sidebar.selectbox("Language / भाषा", ["en", "hi", "mr"],
...
if menu == "Overview Dashboard":
    ...
    data = fetch_api("sarpanch/dashboard/overview")
    ...
    if data:
        ...
        with c1:
            ...
            fig_gender = px.pie(gender_data, names="Gender", values="Count", color="Gender",
        ...
        with c2:
            ...
            if by_cat:
                ...
                fig_cat = px.bar(cat_df, x="Category", y="Count", color="Count", color_continuous_scale="Blues")
...
# Population & Demographics
    ...
    pop_data = fetch_api("sarpanch/analytics/population")
    if pop_data:
        ...
        st.markdown("### Socioeconomic Classification")
        socio_data = {
        ...
        fig_socio = px.pie(socio_data, names="Category", values="Count", hole=0.4)
        st.plotly_chart(fig_socio, use_container_width=True)

# Ward Analytics
elif menu == "Ward Analytics":
    st.title("🏘️ Ward-wise Development Indicators")

    wards = fetch_api("sarpanch/analytics/wards")
    if wards:
        ...
        with c1:
            ...
            fig_wpop = px.bar(ward_df, x="ward_name", y="population", color="population")
...
# AI Decision Support
    ...
    with col1:
        if st.button("🚀 Run Multi-Agent Village Analysis", type="primary"):
            with st.spinner("Analyzing village data across domains..."):
                res = requests.post(f"{API_BASE_URL}/sarpanch/ai/analyze")
                if res.status_code == 200:
                    st.session_state['agent_analysis'] = res.json()
                    st.success("Analysis complete!")

    with col2:
        if st.button("💡 Generate AI Recommendations for Issues"):
            with st.spinner("Generating explainable recommendations..."):
                res = requests.post(f"{API_BASE_URL}/sarpanch/ai/recommendations/generate")
                if res.status_code == 200:
                    st.success(res.json().get("message", "Generated!"))
    ...
    recs_data = fetch_api("sarpanch/ai/recommendations")
    if recs_data:
        ...
        if recs_list:
            for rec in recs_list:
                ...
                <div style="padding:15px; border-radius:8px; border:1px solid #ddd; margin-bottom:15px; background-color:#f9f9f9;">
                    ...
                    <p><b>Status:</b> {rec.get('status')} | <b>Date:</b> {rec.get('created_at')}</p>
                </div>
...
# Government Schemes
    ...
    search_query = st.text_input("🔍 Search schemes by problem keywords (e.g., 'water pipeline', 'school dropout', 'farmer loan')")

    if search_query:
        search_res = fetch_api(f"schemes/search?query={search_query}")
        if search_res:
...
# Reported Issues
elif menu == "Reported Issues":
    st.title("📝 Village Complaints & Issues")

    issues = fetch_api("sarpanch/issues/all")
    if issues:
        issues_df = pd.DataFrame(issues)
        st.dataframe(issues_df, use_container_width=True)

# Panchayat Reports
elif menu == "Panchayat Reports":
    st.title("📄 Panchayat Meeting Report Generator")

    report = fetch_api("sarpanch/reports/summary")
    if report:
